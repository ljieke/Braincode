...
需要环境变量 BRAINCODE_TEST_API_KEY、BRAINCODE_TEST_BASE_URL、BRAINCODE_TEST_MODEL。
...
def test_lock_first_acquire():
    from braincode.memory.consolidation import _read_last_consolidated_at, _try_acquire_lock
...
def test_lock_blocks_when_held():
    from braincode.memory.consolidation import _try_acquire_lock
...
def test_lock_rollback_deletes_on_zero():
    from braincode.memory.consolidation import _try_acquire_lock, _rollback_lock
...
def test_prompt_contains_all_phases():
    from braincode.memory.consolidation import _build_consolidation_prompt
...
@pytest.mark.skipif(
    not os.environ.get("BRAINCODE_TEST_API_KEY"),
    reason="BRAINCODE_TEST_API_KEY not set"
)
@pytest.mark.timeout(120)
def test_e2e_consolidation_merges_duplicates():
    api_key = os.environ["BRAINCODE_TEST_API_KEY"]
    base_url = os.environ.get("BRAINCODE_TEST_BASE_URL", "https://api.minimaxi.com/v1")
    model = os.environ.get("BRAINCODE_TEST_MODEL", "MiniMax-M3")

    with tempfile.TemporaryDirectory() as work_dir:
        mem_dir = os.path.join(work_dir, ".braincode", "memory")
...
async def _run_consolidation(work_dir, api_key, base_url, model, mem_dir):
    ...
    from braincode.permissions.checker import PermissionChecker
    ...
    from braincode.config import ProviderConfig
    from braincode.client import OpenAICompatClient
    ...
    from braincode.permissions.sandbox import PathSandbox
    from braincode.permissions.rules import RuleEngine
    from braincode.permissions.dangerous import DangerousCommandDetector
    from braincode.permissions.checker import PermissionMode
