...
from braincode.conversation import (
    ConversationManager,
    Message,
    ToolResultBlock,
    ToolUseBlock,
...
from braincode.memory.instructions import (
    MAX_INCLUDE_DEPTH,
    load_instructions,
    process_includes,
)
from braincode.memory.session import (
    RecordType,
    ResumeResult,
    Session,
    SessionManager,
...
class TestLoadInstructions:
    def test_single_layer(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        braincode_md = tmp_path / "BRAINCODE.md"
        braincode_md.write_text("project instructions", encoding="utf-8")
    ...
    def test_multi_layer_priority(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        """对齐 Go 版发现顺序：BRAINCODE.md 在前，.braincode/INSTRUCTIONS.md（legacy）在后。"""
        root_md = tmp_path / "BRAINCODE.md"
        root_md.write_text("root level", encoding="utf-8")
        dotdir = tmp_path / ".braincode"
        dotdir.mkdir()
        # Go 版不发现 .braincode/BRAINCODE.md，只发现 .braincode/INSTRUCTIONS.md（legacy）
...
class TestSession:
    def test_append_writes_jsonl_and_updates_meta(self, tmp_path: Path) -> None:
        sessions_dir = tmp_path / ".braincode" / "sessions"
...
class TestMemoryManager:
    ...
    def test_load_includes_memory_index(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        # 创建项目级记忆目录和文件
        project_mem_dir = tmp_path / "project" / ".braincode" / "memory"
    ...
    def test_load_all(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        # 用户级记忆
        user_mem_dir = fake_home / ".braincode" / "memory"
...
class TestConversationInjection:
    def test_inject_long_term_memory(self) -> None:
        ...
        assert "braincodeMd" in conv.history[1].content
    ...
    def test_inject_instructions_only(self) -> None:
        ...
        assert "braincodeMd" in conv.history[0].content
...
class TestMemoryExtraction:
    def test_memory_types_aligned_with_go(self, tmp_path: Path) -> None:
        """验证四种记忆类型与 Go 版一致。"""
        from braincode.memory.auto_memory import VALID_TYPES, _USER_LEVEL_TYPES, _PROJECT_LEVEL_TYPES
