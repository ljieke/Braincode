...
def load_instructions(project_root: str) -> str:
    ...
    发现顺序（低优先级在前，高优先级在后）：
    1. 用户全局：~/.braincode/BRAINCODE.md, ~/.braincode/AGENTS.md
    2. 项目目录链：从 git root 到 workDir，每个目录的 BRAINCODE.md 和 AGENTS.md
    3. workDir/.braincode/INSTRUCTIONS.md（遗留格式）
    4. workDir/BRAINCODE.local.md（本地覆盖）
    """
    ...
    # 1. 用户全局
    _add(home / ".braincode" / "BRAINCODE.md")
    _add(home / ".braincode" / "AGENTS.md")
    ...
    # 3. 遗留格式
    _add(root / ".braincode" / "INSTRUCTIONS.md")

    # 4. 本地覆盖
    _add(root / "BRAINCODE.local.md")
