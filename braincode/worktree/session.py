...
def _session_path(braincode_dir: Path) -> Path:
    return braincode_dir / SESSION_FILENAME
...
def save_worktree_session(
    ...
    path = _session_path(braincode_dir)
    if session is None:
...
def load_worktree_session(braincode_dir: Path) -> WorktreeSession | None:
    path = _session_path(braincode_dir)
    if not path.exists():
        return None
    try:
