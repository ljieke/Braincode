...
def load_config(path: Path | None = None) -> AppConfig:
    ...
    home = Path.home()
    candidates = [
        home / ".braincode" / "config.yaml",
        cwd / ".braincode" / "config.yaml",
        cwd / ".braincode" / "config.local.yaml",
    ]
    ...
    if merged is None:
        raise ConfigError(
            "No config file found. Expected .braincode/config.yaml "
            "in project or ~/.braincode/config.yaml"
        )
    return merged
